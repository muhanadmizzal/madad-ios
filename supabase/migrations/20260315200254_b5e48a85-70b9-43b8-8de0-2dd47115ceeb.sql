...
-- 1. Add unique constraint on tenant_addons for idempotent addon operations
CREATE UNIQUE INDEX IF NOT EXISTS idx_tenant_addons_company_addon 
  ON public.tenant_addons (company_id, addon_key) 
  WHERE status = 'active';

-- 2. Improve approve_addon_request to handle re-activation and use proper ON CONFLICT
...
BEGIN
  ...
  IF p_action = 'approved' THEN
    -- Check if addon already exists (active or inactive)
    SELECT * INTO v_existing FROM tenant_addons 
      WHERE company_id = v_req.company_id AND addon_key = v_req.addon_key;

    IF v_existing.id IS NOT NULL THEN
      -- Re-activate existing addon with updated price
      UPDATE tenant_addons SET 
        status = 'active', 
        custom_price = COALESCE(p_custom_price, v_req.requested_price, v_existing.custom_price),
        billing_cycle = COALESCE(v_req.billing_cycle, v_existing.billing_cycle),
        updated_at = now()
...
-- 3. Enhance get_feature_access to include addon-sourced features in the output
-- and add quota_exceeded flag when quotas are hit
...
BEGIN
  ...
  -- Get user roles regardless of subscription status
  ...
  -- Non-active subscriptions: return early with status info
  ...
  -- 5. Apply user-level AI overrides if user_id provided
  IF p_user_id IS NOT NULL THEN
    DECLARE
  ...
  -- 6. Check quota-based limits and mark features as quota_exceeded
