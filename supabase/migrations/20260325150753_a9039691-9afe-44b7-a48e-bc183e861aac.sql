...
-- Drop and recreate get_feature_access with optional p_position_id parameter
...
BEGIN
  ...
  -- Layer 5: Position feature assignments (NEW)
  -- Resolve position: use provided p_position_id, or look up from employee if p_user_id given
  v_resolved_position_id := p_position_id;
  IF v_resolved_position_id IS NULL AND p_user_id IS NOT NULL THEN
    SELECT position_id INTO v_resolved_position_id FROM employees
      WHERE user_id = p_user_id AND company_id = p_company_id AND status = 'active'
      LIMIT 1;
  END IF;

  IF v_resolved_position_id IS NOT NULL THEN
    FOR v_pos_assign IN SELECT * FROM position_feature_assignments
      WHERE position_id = v_resolved_position_id AND enabled = false
    LOOP
      -- Only restrict if feature is currently enabled (don't override plan blocks)
      v_entry := v_features->v_pos_assign.feature_key;
      IF v_entry IS NOT NULL AND (v_entry->>'enabled')::boolean = true THEN
        v_features := v_features || jsonb_build_object(v_pos_assign.feature_key,
          jsonb_build_object('enabled', false, 'source', 'position', 'reason', 'position_restricted'));
      END IF;
    END LOOP;
  END IF;

  -- Layer 6: User-level AI overrides
  IF p_user_id IS NOT NULL THEN
    DECLARE
  ...
  -- Layer 7: Platform-wide disables
