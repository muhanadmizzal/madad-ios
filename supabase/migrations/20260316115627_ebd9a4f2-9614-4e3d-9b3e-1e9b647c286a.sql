-- FIX 1: auto_generate_document_on_approval references lr.days_count which doesn't exist
-- Replace with calculated value (end_date - start_date + 1)
...
BEGIN
  ...
  CASE v_instance.request_type
    WHEN 'leave' THEN
      ...
      -- FIX: Calculate days from dates instead of referencing non-existent days_count
...
-- FIX 2: system_manage_request_docs is too broad (ALL = includes SELECT for any company member)
-- Drop it and replace with INSERT/UPDATE only for the trigger (SECURITY DEFINER handles inserts)
DROP POLICY IF EXISTS "system_manage_request_docs" ON request_documents;

-- The trigger function runs as SECURITY DEFINER so it bypasses RLS.
-- We only need explicit INSERT/UPDATE policies for non-trigger cases.
-- Add an INSERT policy for the trigger (which runs as SECURITY DEFINER, so this is optional but safe)
CREATE POLICY "hr_manage_request_docs_insert" ON request_documents
...
CREATE POLICY "hr_manage_request_docs_update" ON request_documents
...
-- FIX 3: Remove duplicate SELECT policy on generated_documents
DROP POLICY IF EXISTS "Employees can view own released documents" ON generated_documents;