...
-- 1. Add completed_at column to workflow_instances (referenced by process_approval_action)
ALTER TABLE public.workflow_instances ADD COLUMN IF NOT EXISTS completed_at timestamptz;

-- 2. Add auto_generate_doc_type text column to workflow_templates (referenced by process_approval_action)
ALTER TABLE public.workflow_templates ADD COLUMN IF NOT EXISTS auto_generate_doc_type text;

-- 3. Fix the create_workflow_instance to use approved_at as fallback alongside completed_at
-- (The function already uses completed_at which now exists, so no function change needed)

-- 4. Recreate process_approval_action with fixed column references
...
BEGIN
  ...
  -- Verify actor is current approver or HR
    ...
    WHEN 'approve' THEN
      ...
      IF v_next_step IS NOT NULL THEN
        ...
        ELSIF v_next_step.routing_mode = 'role' AND v_next_step.approver_role IS NOT NULL THEN
          IF v_next_step.approver_role = 'manager' AND v_requester_pos IS NOT NULL THEN
            SELECT r.approver_user_id, r.approver_position_id, r.approver_name
            INTO v_next_approver_uid, v_next_approver_pos, v_next_approver_name
          ...
          ELSE
            ...
            IF v_next_approver_uid IS NOT NULL THEN
              SELECT name_ar INTO v_next_approver_name FROM employees WHERE user_id = v_next_approver_uid AND company_id = v_company_id LIMIT 1;
            END IF;
          END IF;
      ...
      ELSE
        ...
        UPDATE workflow_instances SET status = 'approved', approved_at = now(), completed_at = now(), final_comments = p_comments, updated_at = now() WHERE id = p_instance_id;
        PERFORM sync_approval_to_source(v_instance.request_type, v_instance.reference_id, 'approved');
      END IF;

    WHEN 'reject' THEN
      v_to_status := 'rejected';
      UPDATE workflow_instances SET status = 'rejected', rejected_at = now(), completed_at = now(), final_comments = p_comments, updated_at = now() WHERE id = p_instance_id;
      PERFORM sync_approval_to_source(v_instance.request_type, v_instance.reference_id, 'rejected');

    WHEN 'return' THEN
      v_to_status := 'returned';
      UPDATE workflow_instances SET status = 'returned', completed_at = now(), final_comments = p_comments, updated_at = now() WHERE id = p_instance_id;
  ...
  IF v_to_status = 'approved' THEN
    ...
    IF v_template IS NOT NULL AND v_template.auto_generate_document = true AND v_template.auto_generate_doc_type IS NOT NULL THEN
      BEGIN
...
-- 5. Also fix create_workflow_instance to handle 'manager' role via hierarchy
...
BEGIN
  ...
  -- Find template: exact type → category prefix → general
  ...
  -- If still no template found, raise a clear error
  IF v_template_id IS NULL THEN
    RAISE EXCEPTION 'لا يوجد قالب سير عمل مفعل لنوع الطلب: %', p_request_type;
  END IF;
  ...
  -- B. Role-based (including 'manager' which uses hierarchy)
