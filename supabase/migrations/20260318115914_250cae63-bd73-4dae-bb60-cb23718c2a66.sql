...
BEGIN
  ...
  END IF;
  IF v_signatory IS NULL OR v_signatory.id IS NULL THEN
  ...
  -- Extract signatory info safely (fallback to company-level signatory)
  IF v_signatory IS NOT NULL AND v_signatory.id IS NOT NULL THEN
    v_sig_id := v_signatory.id;
    v_sig_name := COALESCE(v_signatory.name_ar, v_signatory.name);
    v_sig_role := COALESCE(v_signatory.role_ar, v_signatory.role);
  ELSE
    v_sig_id := NULL;
