...
-- Fix process_approval_action: improve employee lookup for auto-generated certificates
-- The current lookup only matches on user_id which many employees don't have
      ...
      ELSE
        ...
        IF v_instance.request_type LIKE 'certificate%' OR v_instance.request_type IN ('document', 'general') THEN
          ...
          IF v_instance.request_type LIKE 'certificate%' THEN
            BEGIN
              -- Get employee info - try user_id first, then fall back to requester from approval_requests
              SELECT e.* INTO v_employee
              FROM employees e WHERE e.user_id = v_instance.requester_user_id AND e.company_id = v_company_id AND e.status = 'active' LIMIT 1;

              -- Fallback: look up employee via approval_requests.requester_id (which may be a user_id)
              ...
              -- Fallback: look up employee from record_id in approval_requests (could be employee_id)
              IF v_employee IS NULL THEN
                SELECT e.* INTO v_employee
                FROM approval_requests ar
              ...
              -- Last fallback: find ANY active employee for this user via profiles
              IF v_employee IS NULL THEN
                SELECT e.* INTO v_employee
                FROM employees e
              ...
              IF v_employee IS NOT NULL AND v_company IS NOT NULL THEN
                -- Determine certificate type from request_type
                ...
                ELSIF v_cert_type = 'employment' THEN
                  v_cert_content := 'تعريف بالراتب' || E'\n\n' ||
                    ...
                    E'\n\n' || 'التاريخ: ' || to_char(now(), 'YYYY-MM-DD');
                ELSE
                  v_cert_content := 'شهادة' || E'\n\n' ||
                    'نشهد نحن ' || COALESCE(v_company.name_ar, v_company.name, '') || ' بأن السيد/السيدة ' ||
                    COALESCE(v_employee.name_ar, '') || ' يعمل/تعمل لدينا.' ||
                    E'\n\n' || 'التاريخ: ' || to_char(now(), 'YYYY-MM-DD');
                END IF;

                -- Insert generated_documents record
                INSERT INTO generated_documents (
                  company_id, employee_id, document_type, content, status, generated_by,
                  visibility_scope, metadata, version, mime_type, workflow_instance_id,
                  signatory_name_snapshot, signatory_role_snapshot,
                  reference_number
                ) VALUES (
                  v_company_id, v_employee.id, v_cert_type, v_cert_content, 'approved', v_actor_user_id,
                  'employee', jsonb_build_object('request_type', v_instance.request_type, 'auto_generated', true),
                  1, 'text/html', p_instance_id,
                  v_company.signatory_name, v_company.signatory_title,
                  'CERT-' || to_char(now(), 'YYMM') || '-' || lpad(floor(random()*9999+1)::text, 4, '0')
                ) RETURNING id INTO v_generated_doc_id;
              END IF;
            EXCEPTION WHEN OTHERS THEN
              RAISE WARNING 'Auto-generate certificate failed: %', SQLERRM;
            END;
          END IF;
        END IF;

        -- Also sync documents table if reference was a document
